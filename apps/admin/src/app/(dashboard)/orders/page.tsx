...
export default function OrdersPage() {
  ...
  const bulkActions: BulkAction[] = React.useMemo(() => {
    ...
    actions.push({
      ...
      tooltip: selectedRows.length === 1 ? undefined : 'Cancel one order at a time',
      onClick: () => selectedRows[0] && setCancelOrderId(selectedRows[0].id),
    });
    return actions;
  }, [sameStatus, selectedRows, onAdvance, canAdvanceStatus]);
  ...
  return (
    <>
      <PageHeader
        ...
        bulk={
          <BulkActionBar
            ...
            actions={bulkActions}
            meta={selected.size > 0 ? `${formatMoney(selectedTotal, currency)} selected` : null}
          />
        }
      />
          ...
          <div>
            <div className="text-sm text-fg-muted">
              {filters.status === 'all' && !debouncedSearch && filters.types.length === 0
                ? 'No orders yet. New ones will appear here in real time.'
      ...
      <ActionModal
        ...
        title={`Advance ${selectedRows.length} orders?`}
        description="Each selected order will be moved to its next status. This cannot be undone in bulk."
        primary={{
          label: `Advance ${selectedRows.length}`,
          onClick: runBulkAdvance,
        }}
        secondary={{ label: 'Cancel', onClick: () => setConfirmBulkAdvance(false) }}
      />
    </>
  );
}

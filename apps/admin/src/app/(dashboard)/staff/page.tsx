    ...
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          if (!canWrite || isSelf) {
            return <span className="text-fg-muted">{role}</span>;
          }
          return (
            <select
              value={role}
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          const label = !verified ? 'Invited' : active ? 'Active' : 'Disabled';
          const cls = !verified
  ...
  return (
    <RequirePermission perm={['staff:read', 'staff:write']} mode="any">
      <DataTable
        ...
        rowKey={(r) => r.id}
        loading={q.isLoading}
        emptyState={<div className="text-sm text-fg-muted">No staff members yet.</div>}
      />

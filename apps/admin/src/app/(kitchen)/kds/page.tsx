    ...
    <div className="flex h-screen flex-col">
      <header className="flex shrink-0 items-center justify-between border-b border-border/[var(--border-alpha)] bg-surface/60 px-6 py-3 backdrop-blur">
        <div className="flex items-center gap-3 text-small">
          ...
          <h1 className="text-h2 font-semibold text-fg">Kitchen Display</h1>
          <span className="text-fg-subtle">·</span>
          <span className="tabular-nums text-fg-muted">
            <span className="text-fg">{all.length}</span> tickets
            {lateCount > 0 && (
              <>
                {' '}
                · <span className="text-negative">{lateCount} late</span>
              </>
            )}
          </span>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button
            type="button"
            aria-pressed={sound}
            aria-label={sound ? 'Mute alerts' : 'Unmute alerts'}
      ...
      {feed.isLoading ? (
        ...
        <div className="grid flex-1 place-items-center px-6">
          <EmptyState
            ...
            description={(feed.error as Error)?.message ?? 'Check connection.'}
        ...
        <div className="grid min-h-0 flex-1 grid-cols-1 gap-4 p-4 lg:grid-cols-3">
          {(['CONFIRMED', 'PREPARING', 'READY'] as const).map((status) => {
            ...
            return (
              <Column key={status} title={status} count={list.length} empty="No tickets">
                {list.map((t) => (
                  <KdsTicketCard
                    key={t.orderId}
